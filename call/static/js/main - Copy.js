...
const ws = new WebSocket('ws://localhost:8888/kurento');
...
function uiStart()
{
      ...
      function(err)
  {
    ...
    webRtcPeer.generateOffer((err, sdpOffer) => {
      ...
      $.post("http://localhost:8080/send_offer/",{"sdp_offer":sdpOffer})
...
function showSpinner()
{
  for (let i = 0; i < arguments.length; i++) {
    arguments[i].poster = '/static/img/transparent-1px.JPG';
    arguments[i].style.background = "center transparent url('/static/img/spinner.JPG') no-repeat";
  }
}
...
function hideSpinner()
{
  for (let i = 0; i < arguments.length; i++) {
    arguments[i].src = '';
    arguments[i].poster = '/static/img/webrtc.JPG';
